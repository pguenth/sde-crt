...
valuesp = {}
histogramp = {}
histogramp_sameedges = {}
histogramp_sameedges_norm = {}
bins = None
for T_ in obs_at[::-1]:
    ...
    histogramp_sameedges[T_] = HistogramNode(f'histop_sameedges_{T_}', {'values' : valuesp[T_]['values'], 'weights' : valuesp[T_]['weights']}, log_bins=True, normalize=None, **(histo_opts | {'edges' : bins}))
    histogramp_sameedges_norm[T_] = HistogramNode(f'histop_sameedges_norm{T_}', {'values' : valuesp[T_]['values'], 'weights' : valuesp[T_]['weights']}, log_bins=True, normalize='density', **(histo_opts | {'edges' : bins}))
    histogramp[T_] = HistogramNode(f'histop_{T_}', {'values' : valuesp[T_]['values'], 'weights' : valuesp[T_]['weights']}, log_bins=True, normalize=None, **(histo_opts | {'bin_count' : bin_count}))

meshgrid = MeshgridTimeseriesNode("meshgrid", histogramp, plot='mesh', cache=cache, ignore_cache=False, plot_kwargs={'label': '$\\log N$', 'levels': 20, 'transpose': True})
greens = GreensFunction("green", histogramp_sameedges, plot='mesh', cache=cache, ignore_cache=False, plot_kwargs={'label': '$\\log N$', 'levels': 20, 'transpose': True})
...
def impulse_response_test(source_name, sourcefunction):
    ...
    # colvolution
    test_inj = InjectionConvolveHistogram("testinj", greens, plot='inj', cache=cache_test, source_callback=sourcefunction_norm, ignore_cache=True)
...
nfig3d_2.add(greens, 0, plot_on='mesh')
nfig3d_2.savefig(f"{figdir}/{name}_3d_const_pbins.pdf")

nfig3d = NodeFigure(format_mesh)
nfig3d.add(meshgrid, 0, plot_on='mesh')
nfig3d.savefig(f"{figdir}/{name}_3d.pdf")
#

...
def datapoint(delta_inv, dx_adv, sigma, store_histogram=True):
    ...
    n_timesteps = n_timesteps_per_delta_inv * delta_inv**2 #! IMPORTANT: with this, we reproduce remarkably similar figures to the dxadv_deltainv figures. why is this?
...
allrows = rows(delta_invs=[1.01, 1.1, 1.2, 1.5, 1.8, 2.4, 3.0, 3.2, 3.4, 3.6], dx_advs=[0.005], sigma=0.95)#, 0.05, 0.1, 0.5], sigma=0.95)

xlabel = '\\delta^{-1} = \\Delta x_\\textrm{diff}/\\Delta x_\\textrm{adv}'
nfig = NodeFigure(formats.powerlaws, suptitle='Downstream transport in $T/\\Delta\\tau = {$', xlabel='$' + xlabel + '$', ylabel="$x / \Delta x_\\textrm{{adv}}$")
for dx_adv, row in allrows.items():
    nfig.add(row, 0)
nfig.pad(.2)
nfig[0].annotate('$\\Delta\\tau =' + str(common_parameters['dt']) + '$\n$\\sigma=0.95$', (0.50, 0.18), xycoords='figure fraction', bbox=dict(boxstyle="square,pad=0.5", fc="white", ec="black", lw=0.5))

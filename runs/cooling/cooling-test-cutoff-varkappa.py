...
def webb84_from_params(params):
    # for q = beta**2 / kappa on both sides
...
def k_syn_from_cutoff_webb84(cutoff, r, q):
    # for q = beta**2 / kappa on both sides
    return 1 / (6 * cutoff * q * r) * (r - 1) 

cachedir = "cache"
figdir = "figures"

name = "cooling_test_cutoff_varkappa"
...
def do(init, rname):
    ...
    for k_syn, split_lim1, split_lim2 in runs:
        ...
        solvernode = SDESolverNode(f'solver_{rname}_{k_syn}', sde=sde, scheme=b'semiimplicit_weak', timestep=dt, observation_times=obs_T, nthreads=64, cache=cache, splitted=True)

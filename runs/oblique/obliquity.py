...
def diffusion(out, t, x, Xsh, a, b, q, kappa_perp, alpha_u, alpha_d):
    # cpp: kruells_shockaccel2_diffusion
    kappa_par = np.sqrt(2.0 * kruells94_kappa_dep(x[0], Xsh, a, b, q))
    ...
    kappa_xx = ksum - c2a * kdiff
    kappa_zz = ksum + c2a * kdiff
    kappa_xz = s2a / 2 * kdiff

    # here carray is required to reshape the contiguous pointer
    out_a = carray(out, (3, 3))
    out_a[0, 0] = np.sqrt(2.0 * kappa_zz)
    out_a[1, 0] = np.sqrt(2.0 * kappa_xz)
    out_a[2, 0] = 0
    out_a[0, 1] = np.sqrt(2.0 * kappa_xz)
    out_a[1, 1] = np.sqrt(2.0 * kappa_xx)
    out_a[2, 1] = 0
    out_a[0, 2] = 0
    out_a[1, 2] = 0
    out_a[2, 2] = 0
